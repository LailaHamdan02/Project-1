...
int main(int argc, char** argv) {
	...
	while ( !game_over ) {
		...
		while ( (!guessed_code) && (num_guesses < MAX_GUESSES) ) {
            ...
            string player_code_copy = player_code;
            string secret_code_copy = secret_code;
           
			for (int i = 0; i < CODE_LENGTH; ++i) {
   
                if (secret_code_copy[i] == player_code_copy[i]){
   			...
   			for (int i = 0; i < CODE_LENGTH; ++i) {        
   				for (int j=0; j < CODE_LENGTH; ++j){
       				if (secret_code_copy[i] == player_code_copy[j]){
           
           				secret_code_copy[i] = 'y';
                        player_code_copy[j] = 'x';
		...
		/* Otherwise, flip the "guessed code" flag again to start a new round */
   			...
   			//game_over=false added to make sure that when entered Yes it still ran.
